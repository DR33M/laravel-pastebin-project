...
@section('content')
    <form action="{{ route('update-note', $note->token) }}" method="POST">
        @csrf
        @method('PATCH')

        <div class="content__title">
            Edit Paste: {{ $note->title }}
        </div>
        <div class="form-group field-postform-text required">
            <textarea id="postform-text" class="textarea @error('body') is-invalid @enderror" name="body"  style="overflow: hidden; overflow-wrap: break-word; min-height: 300px;">{{ $note->body }}</textarea>
            @error('body')
        ...
        <div class="post-form__bottom">
            <div class="post-form__left">
                    ...
                    <div class="col-sm-9 field-wrapper">
                        <select id="postform-format" class="postform-format form-control @error('syntax_id') is-invalid @enderror" name="syntax_id">
                            @foreach($syntax as $item)
                                <option value="{{ $item->id }}" {{ $note->syntax_id == $item->id ? 'selected' : '' }}>{{ ucfirst($item->name) }}</option>
                            @endforeach
                        </select>
                        @error('syntax_id')
                            <span class="invalid-feedback" role="alert">
                    ...
                    <div class="col-sm-9 field-wrapper">
                        <select id="postform-status" class="postform-status form-control @error('status_id') is-invalid @enderror" name="status_id">
                            @foreach($status as $item)
                                <option value="{{ $item->id }}" {{ $note->status_id == $item->id ? 'selected' : '' }}>{{ ucfirst($item->name) }}</option>
                            @endforeach
                        </select>
                        @error('status_id')
                            <span class="invalid-feedback" role="alert">
                ...
                <div class="form-group field-postform-name d-flex">
                    ...
                    <div class="col-sm-9 field-wrapper">
                        <input type="text" id="postform-name" class="postform-name form-control @error('title') is-invalid @enderror" name="title" value="{{ $note->title }}" maxlength="255">
                        @error('title')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                ...
                <div class="form-group form-btn-container d-flex">
                    <button type="submit" class="btn -big">Edit Paste</button>
                </div>
            </div>
        </div>
    </form>
